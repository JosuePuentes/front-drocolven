...
function filtrarPorMultiplesPalabrasAND<T>(
  ...
  const palabrasBusqueda = textoBusqueda
    .toLowerCase()
    .split(" ")
    .filter(Boolean);
...
export const CarritoCompras = () => {
  ...
  const productosFiltrados = filtrarPorMultiplesPalabrasAND(
    productos,
    busqueda,
    ["descripcion"]
  );
  ...
  return (
    <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 relative mt-12">
            ...
            <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex justify-center items-center">
              <div className="bg-white w-11/12 max-h-[80vh] overflow-y-auto rounded-2xl p-4 shadow-xl relative">
                ...
                <ResumenCarrito
                  carrito={carrito}
                  onSumar={(id) =>
                    agregarProducto(carrito.find((p) => p.id === id)!)
                  }
                  onRestar={quitarProducto}
