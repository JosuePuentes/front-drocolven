...
export const PedidosOrganizados = () => {
  const [ventas, setVentas] = useState<PedidoArmado[]>([]); // Usar el tipo PedidoArmado
  ...
  useEffect(() => {
    const fetchVentas = async () => {
      try {
        ...
        const data: PedidoArmado[] = await response.json(); // Usar el tipo PedidoArmado
        ...
        <>
          ...
          <div className="mt-6 max-h-96 overflow-y-auto border rounded-lg p-4 bg-gray-50">
            <ul className="space-y-3">
              {pedido.productos.map((p) => {
                const correcto = p.cantidadEncontrada === p.cantidad_pedida;
                const incompleto = p.cantidadEncontrada < p.cantidad_pedida;
                const sobrante = p.cantidadEncontrada > p.cantidad_pedida;
