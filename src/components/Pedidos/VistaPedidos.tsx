...
import { AiOutlineWarning, AiOutlineCalendar } from "react-icons/ai";
import { usePedido } from "../hooks/usePedido";

const ESTADOS = [
  { id: 'nuevo', label: 'Nuevo' },
  { id: 'picking', label: 'Picking' },
  { id: 'checkpicking', label: 'Check Picking' },
                      ...
                      <li key={key} className="py-2 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                        <div className="flex-1">
                          <span className="font-medium text-gray-900 flex items-center gap-2">
                            {prod.descripcion}
                            {prod.frio && (
                              <MdAcUnit className="w-4 h-4 text-blue-600" title="Frío" />
                            )}
                            {prod.advertencia && (
                              <AiOutlineWarning className="w-4 h-4 text-purple-600" title="Advertencia" />
                            )}
                            {prod.limpieza && (
                              <FaLeaf className="w-4 h-4 text-amber-700" title="Limpieza" />
                            )}
                            {prod.calendario && (
                              <AiOutlineCalendar className="w-4 h-4 text-green-600" title="Calendario" />
                            )}
                          </span>
