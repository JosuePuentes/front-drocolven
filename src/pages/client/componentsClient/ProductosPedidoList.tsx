...
import { ProductoPedido } from './PedidoTypes';

interface ProductosPedidoListProps {
  productos: ProductoPedido[];
}

const ProductosPedidoList: React.FC<ProductosPedidoListProps> = ({ productos }) => (
    ...
    {productos.map((prod) => (
      ...
      >
        <div className="flex flex-col w-full sm:w-auto">
          ...
          <span className="text-xs sm:text-sm text-gray-500">Cantidad: {prod.cantidad_pedida}</span>
        </div>
        <div className="flex flex-col sm:flex-col items-start sm:items-end w-full sm:w-auto justify-between sm:justify-end gap-1 sm:gap-0 mt-1 sm:mt-0">
          ...
          {[prod.descuento1, prod.descuento2, prod.descuento3, prod.descuento4].some(Boolean) && (
            <span className="inline-flex items-center py-0.5  text-green-700 text-xs font-medium whitespace-nowrap">
              Descuentos: {[prod.descuento1, prod.descuento2, prod.descuento3, prod.descuento4].filter(Boolean).join('% + ')}%
            </span>
          )}
          <span className="text-sm sm:text-base font-semibold text-blue-700 whitespace-nowrap">Total: $ {prod.precio.toFixed(2)}</span>
        </div>
      </div>
    ))}
  </div>
